...
with tf.Session() as sess:
    print(sess.run(init))
    writer = tf.summary.FileWriter("./DeepLearning_Lesson4", sess.graph)
    print('Training...')
## changed range size to 15000
    for i in range(15001):
        batch_xs, batch_ys = mnist.train.next_batch(128)
        _, c, summary = sess.run([train_step, loss, merged_summary_op], feed_dict={x: batch_xs, y_: batch_ys})
